...
export default function App() {
  ...
  const execJobAction = () => {
    ...
    if(type==="delete") {
      ...
      setJobs(prev=>prev.filter(j=>j.id!==job.id));
      setJobHistory(prev=>[{...job, archivedDate:todayStr()}, ...prev]);
    }
    setExpanded(null);
  ...
  const restoreJob = (job) => {
    ...
    setJobs(prev=>[{...job, archivedDate:undefined}, ...prev]);
  };
  ...
  const saveForm = () => {
    ...
    setJobs(prev=>[{id:Date.now(),client:form.client,type:form.type||"General Job",amount:parseFloat(form.amount)||0,date:form.date||todayStr(),status:form.status||"unpaid",phone:form.phone||"",recurring:form.recurring||"none",recurringDay:form.recurringDay||"",recurringTime:form.recurringTime||"",paymentDue:form.paymentDue||""}, ...prev]);
  ...
  const handleSettingsClose = (result) => {
    ...
    if(result.action==="signout")   setConfirm({icon:"👋",title:t.signedOut,   msg:t.signedOutMsg});
